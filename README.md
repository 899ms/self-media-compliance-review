...
- 用户没有指定通道时，按上表档位顺位启用：宿主有 ①computer use / ②浏览器工具（`agent-browser` 一类真实浏览工具）就直接用，不打扰用户；宿主没有 ①② 时进入“问用户”环节（③ `opencli` 为第一选项，④ TikHub 花钱需先征得明确同意）；⑤ MediaCrawler 仅当 ①-④ 都不可用时使用。
...
需要实时平台证据时，agent 按以下档位顺位启用（用户点名了某个通道则只用它，不静默换源）：

| 档位 | 数据源 | 启用条件 |
|---|---|---|
| ① | agent 自带 computer use（屏幕操控，像真人一样打开平台页面） | agent 自检宿主能力，有就直接用，不打扰用户 |
| ② | agent 自带浏览器工具（browser use / MCP 浏览器等宿主内置能力） | 同上，①不可用或页面读不出时顺位启用 |
| ③ | `opencli`（本机站点 CLI 适配器） | 宿主没有 ①② 时，进入“问用户”环节的第一选项 |
| ④ | TikHub API（付费、稳定、五平台含视频号） | 问用户选择时可选；花钱必须先征得明确同意 |
| ⑤ | MediaCrawler（最后手段） | 仅当 ①-④ 都不可用；启用前还受频率铁律硬约束（30 分钟冷却、失败退避、全机单实例锁、体量上限） |
...
> 使用 self-media-compliance-review 审核这条笔记，并实时搜索一下近期相似的导流处罚案例。按档位顺位用可用的通道（①② 有就直接用），都不可用就继续静态审核并说明未搜索。
